...
def keras_crnn(imgH, nc, nclass, nh, n_rnn=2, leakyRelu=False,lstmFlag=True):
   ...
   # 作为转换用执行以下代码
   # 在darknet中CRNN模型的最后一层用的是conv，而pytorch和keras中用的是 liner，将keras中的最后一层改为conv，在将pytorch转成keras的时候，转换一下
    out = Conv2D(filters=5530, kernel_size=1,  strides=(1, 1), padding= 'valid', dilation_rate=(1, 1), activation='linear',
                   use_bias=True,data_format=data_format,
                   name='cnn.conv{0}'.format(7)
                   )(x)
