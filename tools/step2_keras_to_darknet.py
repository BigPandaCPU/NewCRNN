...
class DarkNetParser(object):
    ...
    def conv(self, block):
        ...
        conv_layer = (Conv2D(
                ...
                use_bias= True,#not batch_normalize,
        ...
        if activation == 'linear' or activation =='relu':
    ...
    def maxpool(self,block):
        size = int(block['size'])
        stride_list = block['stride'].split(',')
        
        if len(stride_list)>1:
            stride_h = int(stride_list[0])
            stride_w = int(stride_list[1])
        else:
            stride_h = stride_w = int(stride_list[0])
        print('+1',' maxpool, stride',(stride_h, stride_w))
        #stride = int(block['stride'])
